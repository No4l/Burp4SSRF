...
public class BurpExtender implements IBurpExtender {
    private static final String name = "Burp4SSRF";
...
class MetaRequest {
    ...
    private static int burpId = 0;
    private long timestamp;

    MetaRequest(IHttpRequestResponse proxyMessage) {
...
class Injector implements IScannerCheck{
    ...
    public byte[] injectPayloads(byte[] request, Integer requestCode) {
        ...
        for (String[] injection: injectionPoints) {
            if(injection[0].equals("raw") || injection[0].equals("host")){
                continue;
            }
    ...
    public void injectAnyWhere(byte[] request, Integer requestCode, IHttpRequestResponse iHttpRequestResponse) {
        request = Utilities.addOrReplaceHeader(request, "Cache-Control", "no-transform");

        for (String[] injection: injectionPoints) {
            String payload = injection[1].replace("%s", collab.generateCollabId(requestCode, injection[1]));
            // replace %h with corresponding Host header (same as with %s for Collaborator)
            payload = payload.replace("%h", Utilities.getHeader(request, "Host"));
            switch ( injection[0] ){
                case "raw":
                    Utilities.callbacks.makeHttpRequest(iHttpRequestResponse.getHttpService(),Utilities.replaceRequestLine(request,  payload));

                    break;

                case "host":
                    Utilities.callbacks.makeHttpRequest(iHttpRequestResponse.getHttpService(),Utilities.addOrReplaceHeader(request, "Host",payload));
                    break;
            }
    ...
    public List<IScanIssue> doPassiveScan(IHttpRequestResponse iHttpRequestResponse) {
        if (iHttpRequestResponse.getHttpService().getHost().endsWith(collab.getLocation())) {
            return null;
        }

        MetaRequest req = new MetaRequest(iHttpRequestResponse);
        Integer requestCode = collab.addRequest(req);
        Utilities.callbacks.makeHttpRequest(iHttpRequestResponse.getHttpService(),injectPayloads(iHttpRequestResponse.getRequest(), requestCode));
        injectAnyWhere(iHttpRequestResponse.getRequest(), requestCode,iHttpRequestResponse);
        return null;
    }
